...
public class ListDemo {
	...
	public static void listOperate(Jedis jedis, ShardedJedis shardedJedis) {
		...
		jedis.rpush("list", "item1");
		jedis.rpush("list", "item2");
		jedis.rpush("list2", "item3");
		//当列表中不包含任何元素时，阻塞弹出操作会在给定时限内等待可弹出元素的出现，并在时限到达后返回None
		jedis.brpoplpush("list2", "list", 10);
		System.out.println("list：" + shardedJedis.lrange("list", 0, -1));
		//在10秒内阻止并等待可弹出元素的出现
		jedis.brpoplpush("list2", "list", 10);
		System.out.println("list：" + shardedJedis.lrange("list", 0, -1));
	}
