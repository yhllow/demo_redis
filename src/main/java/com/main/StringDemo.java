...
public class StringDemo {
	...
	public static void stringOperate(Jedis jedis, ShardedJedis shardedJedis) {
		...
		//替换内容
		shardedJedis.setrange("key302", 5, "555");
		System.out.println("替换key302对应值中的子串：" + jedis.get("key302"));
		
		jedis.set("key010", "1");
		System.out.println("获取key010对应值中的串：" + jedis.get("key010"));
		//值加1
		jedis.incr("key010");
		System.out.println("获取key010对应值中的串：" + jedis.get("key010"));
		//值加10
		jedis.incrBy("key010", 10);
		System.out.println("获取key010对应值中的串：" + jedis.get("key010"));
		
		jedis.set("key004", "value004");
		jedis.getbit("key004", 2);
		
		System.out.println("=============二进制操作=============");
		//通过将第2个二进制位以及第7个二进制位的值设置为1，键的值将变为"!"，也就是编码为33的字符
		jedis.setbit("another-key", 2, true);
		jedis.setbit("another-key", 7, true);
		System.out.println("获取another-key对应值中的串：" + jedis.get("another-key"));
	}
