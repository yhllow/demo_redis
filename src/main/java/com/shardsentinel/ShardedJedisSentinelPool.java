...
//http://blog.csdn.net/wtyvhreal/article/details/46518375
public class ShardedJedisSentinelPool extends Pool<ShardedJedis> {
    ...
    private List<JedisShardInfo> makeShardInfoList(List<HostAndPort> masters) {
        ...
        for (HostAndPort master : masters) {
            JedisShardInfo jedisShardInfo = new JedisShardInfo(master.getHost(), master.getPort(),
                    timeout);
    ...
    //取得当前所有分片的master地址(IP&PORT)，对每个分片，通过顺次连接Sentinel实例，获取该分片的master地址，如果无法获得，即所有Sentinel都无法连接，将休眠1秒后继续重试，直到取得所有分片的master地址
    ...
    protected static class ShardedJedisFactory implements PooledObjectFactory<ShardedJedis> {
        ...
        public ShardedJedisFactory(List<JedisShardInfo> shards, Hashing algo,
                Pattern keyTagPattern, int database) {
            this.shards = shards;
            this.algo = algo;
        ...
        public void activateObject(PooledObject<ShardedJedis> pooledShardedJedis) throws Exception {
            final BinaryShardedJedis shardedJedis = pooledShardedJedis.getObject();
            for (Jedis jedis : shardedJedis.getAllShards()) {
                    ...
                    jedis.subscribe(new JedisPubSubAdapter() {
                        ...
                        public void onMessage(String channel, String message) {
                            ...
                            if (switchMasterMsg.length > 3) {
                                ...
                                if (index >= 0) {
                                    HostAndPort newHostMaster = toHostAndPort(Arrays.asList(
                                ...
                                log.severe("Invalid message received on Sentinel " + host + ":"
                                        + port + " on channel +switch-master: " + message);
                            }
                        }
                    }, "+switch-master");
